  ...
  IN VOID *Context)
{
  ...
  //   &Mode
  // );
  Status = mineVariableServiceGetVariable (ALICEMODE_VARNAME, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, aliceKey, sizeof(UINTN), &Mode);
  if (EFI_ERROR (Status)) {
  ...
  //Replaced for avoiding hardware dependency
  ReadyToLock(Event, Context);
  ...
  )
{
  ...
  // Create Alice Key
  //
  aliceKey = malloc(sizeof(DEMO1_ACCESS_KEY));
  ...
  Status = mineVariableServiceSetVariable (
    ALICEMODE_VARNAME, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,aliceKey,sizeof(UINTN),&Mode);
  if (EFI_ERROR (Status)) {
   // DEBUG ((DEBUG_ERROR, "%a: variable '%s' could not be written - bailing!\n", 
   //   __FUNCTION__, ALICEMODE_VARNAME));
    return Status;
